...
//If input is to be made from file set FILE_INPUT to 1 and set the correct input file name,
//otherwise set FILE_INPUT for random input
#define FILENAMEIN "myinput.txt"
#define FILE_INPUT 0
...
{
	int generations, dimension, buff;
	...
	if (fpIN == NULL)
	{
		printf("Input file didn't open properly.\n");
		exit(-1);
	}
	...
	for(i = 1; i<=dimension; i++) {
		for(j = 1; j<=dimension + 1; j++) {
			...
			if (buff == '0')
				board[i*(dimension+2)+j] = 0;
			else if ( buff == '1')
				board[i*(dimension+2)+j] = 1;
			else{
				printf("Input includes not permitted chars\n");
				exit(-1);
			}
		}
	}
